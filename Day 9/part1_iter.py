...
"""
The findPaths function is now much more memory efficient, as a generator is used.
This doesn't necessarily speed up the solution.
"""
...
def findPaths(start, path, visited, count, graph):
    ...
    if length == locationCount:
        #Yield the path and the distance
        yield [currPath, count]

    #Recursive case: visit unvisited locations
    else:
        for i in xrange(locationCount):
            if not currVisit[i]:
                for path in findPaths(uniqueLocations[i], currPath, currVisit, count, graph):
                    yield path
...
#Find the smallest distance and path
for start in uniqueLocations:
    visited = [False for location in uniqueLocations]
    visited[locationIndex[start]] = True
    path = []
    count = 0
    for result in findPaths(start, path, visited, count, graph):
        if result[1] < currMin or currMin == -1:
            currMin = result[1]
            resultPath = result[0]
