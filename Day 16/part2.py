...
include = {"children": 3,  "samoyeds": 2, "cars": 2, "perfumes": 1, "akitas": 0, "vizslas": 0}
greater = {"cats": 7, "trees": 3}
lesser = {"pomeranians": 3, "goldfish": 5}
	...
	for compound in auntData[aunt]:
		if compound in include:
			...
			if include[compound] != auntData[aunt][compound]:
				...
				match += 1
		elif compound in greater:
			if greater[compound] >= auntData[aunt][compound]:
				match = -1
				break
			else:
				match += 1
		elif compound in lesser:
			if lesser[compound] <= auntData[aunt][compound]:
				match = -1
				break
			else:
				match += 1
